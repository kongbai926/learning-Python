"""
03 增加了创建目录权限，可在.py文件下创建目录，自定义保存位置。
具体更改保存位置在56行代码。个人比较懒，并且此程序很可能仅供本人使用，不提供输入框输入目录（打开.py文件改目录更直接）
"""
...
def spyder(uil, header):
    ...
    try:
        ...
        wb.save(path + '/天津市各景点信息' + m[3:] + '.xlsx')
        wb.close()
...
try:
    ...
    header = {
        'user-agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) '
                      'Chrome/86.0.4240.198 Safari/537.36'}  # 请求头
    url_list = []
    for i in range(3, 6):
        urlz = uil + str(i) + "a/"
